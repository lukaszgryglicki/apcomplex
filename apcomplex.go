...
func (c *Complex) Close() { if c != nil && c.init { C.mpc_clear(&c.z[0]); c.init = false } }
...
func (c *Complex) SetPrec(bits uint) *Complex {
    if !c.init { panic("apcomplex: not initialized") }
    if bits == 0 { bits = 53 }
    if bits == c.prec { return c }
...
func (c *Complex) Clone() *Complex { out := New(c.prec); C.mpc_set(&out.z[0], &c.z[0], defaultRnd); return out }
...
func Parse(s string, prec uint) (*Complex, error) {
    z := New(prec)
    if err := z.SetString(s); err != nil { z.Close(); return nil, err }
    return z, nil
}

// MustParse panics on error.
func MustParse(s string, prec uint) *Complex { z, err := Parse(s, prec); if err != nil { panic(err) }; return z }
...
func (c *Complex) SetString(s string) error {
    ...
    if C.mpc_set_str(&c.z[0], cs, 10, defaultRnd) != 0 {
        return fmt.Errorf("apcomplex: invalid complex literal %q (normalized %q)", s, t)
    }
    return nil
}

// normalizeInput converts common forms into MPC's "(re im)" string.
...
func (c *Complex) Add(a, b *Complex) *Complex { C.mpc_add(&c.z[0], &a.z[0], &b.z[0], defaultRnd); return c }
func (c *Complex) Sub(a, b *Complex) *Complex { C.mpc_sub(&c.z[0], &a.z[0], &b.z[0], defaultRnd); return c }
func (c *Complex) Mul(a, b *Complex) *Complex { C.mpc_mul(&c.z[0], &a.z[0], &b.z[0], defaultRnd); return c }
func (c *Complex) Div(a, b *Complex) *Complex { C.mpc_div(&c.z[0], &a.z[0], &b.z[0], defaultRnd); return c }
...
func (c *Complex) Pow(a, b *Complex) *Complex { C.mpc_pow(&c.z[0], &a.z[0], &b.z[0], defaultRnd); return c }
...
func Pow(a, b *Complex) *Complex  { p := a.prec; if b.prec > p { p = b.prec }; return New(p).Pow(a, b) }
